...
var VERSION_INFO = { version: "2.2.1", build: 1527869539981 };
  ...
  drawData: function drawData() {
    ...
    this.sortedEntities.forEach(function (bar) {
      ...
      if (force || presentationModeChanged || bar.isNew || bar.changedValue) {
        ...
        }).attr('dx', ".5em").attr('y', _this6.activeProfile.barHeight / 2);
      }
      ...
      if (force || bar.changedWidth || presentationModeChanged) {
        ...
        if (force || bar.changedWidth || presentationModeChanged) {
          ...
          bar.barRank.transition().duration(duration).ease(d3.easeLinear).attr('x', labelX(value) + Math.max(width, bar.barValue.node().getBBox().width + 10));
        }
